...
class ElevatorSystem:
    ...
    def update_status(self, ele_id, update_goal_floor, direction, destination): 
        self.ele_status[ele_id][1].append({"floor": update_goal_floor, "direction": direction, "destination": destination}) # adding floor/direction entry to destination
        self.ele_status[ele_id][2] = np.sign(self.ele_status[ele_id][1][0]["floor"] - self.ele_status[ele_id][0]) # updating elevator direction
        
        #If elevator going up, then sort pick request floors in ascending order, and direction in descending
        ...
        #Else if elevator going down, then sort pick request floors in descending order, and direction in ascending
    ...
    def pick(self, floor_num, direction, destination_floor):
        #Initializing.
        ...
        for i,ele in enumerate(self.ele_status):
            if ele[2] == 0: # If an elevator is static
                if ele[0] == floor_num: # And if elevator is on same floor as request. That elevator is processed to pick up. Return function
                    self.update_status(i, floor_num, direction, destination_floor)
                    return
        ...
        if len(static_elevators_difference) > 0:
            ...
            self.update_status(static_elevators_difference[0]["ele_id"], floor_num, direction, destination_floor)
            return
        ...
        if len(same_direction_moving_elevators) > 0:
            ...
            self.update_status(same_direction_moving_elevators[0]["ele_id"], floor_num, direction, destination_floor)
            return
        ...
        self.update_status(best_ele_opposite_direction_moving_elevators["ele_id"], floor_num, direction, destination_floor)
            ...
            for i,ele in enumerate(self.ele_status): # Loop over all the elevators' statuses
                if len(ele[1]) != 0: # If elevator is not static but moving
                    ...
                    if ele[0] == ele[1][0]["floor"]: # If the elevator reaches the floor of its (first) destination
                        if ele[1][0]["destination"] is not None:
                            ele[1].append({"floor": ele[1][0]["destination"], "direction": ele[1][0]["direction"], "destination": None})
